...
export default function ObjectDetection() {
  ...
  return (
    <div className="p-6">
      ...
      <div className="flex items-center justify-between mb-4 flex-wrap gap-3">
        ...
        {(status === "running" || status === "loading") && (
          <motion.button
            ...
            whileTap={{ scale: 0.95 }}
            onClick={stopWebcam}
            className="px-5 py-2 bg-red-500/10 border border-red-500/30 text-red-500 text-sm font-medium rounded-xl hover:bg-red-500/20 transition-all cursor-pointer"
          >
            Stop Detecting
          </motion.button>
        )}
      ...
      {/* Video area */}
      <div className="relative rounded-xl overflow-hidden bg-black/80" style={{ aspectRatio: "4/3", maxHeight: 480 }}>
        {status === "idle" && (
          <div className="absolute inset-0 flex flex-col items-center justify-center gap-4">
            ...
            <p className="text-sm text-white/30">Point your camera at inventory items</p>
          </div>
        )}
      ...
      {status === "running" && detections.length > 0 && (
        <div className="mt-4 grid grid-cols-2 sm:grid-cols-4 gap-2">
          {detections.slice(0, 8).map((d, i) => (
            <div key={`${d.class}-${i}`} className="px-3 py-2 rounded-lg text-xs bg-white/5">
              <span className="font-medium text-white">{d.class}</span>
